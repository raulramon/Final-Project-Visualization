...
```
```{r}
DNnCatcr_200R <- table1
DNnCatcr_200R <- as.data.frame(DNnCatcr_200R)
DNnCatcr_200R <- mutate(DNnCatcr_200R, tab = "DNnCC")
DNnCatcr_200R 
write.csv(DNnCatcr_200R, "DNnCatcr_200R.csv")
```
...
```{r}
data_frame_1 <- as.data.frame(table1)
```



```{r}
data_frame_1 <- mutate(data_frame_1, tab = "table1")
data_frame_1
```

```{r}
data_frame_2 <- mutate(data_frame_2, tab = "table2")
data_frame_3 <- mutate(data_frame_3, tab = "table3")
```


```{r}
data_frame_2 <- as.data.frame(table2)
data_frame_3 <- as.data.frame(table3)
```

```{r}
write.csv(data_frame_1, "table1_DNNW.csv")
write.csv(data_frame_2, "table2_DNNW.CSV")
write.csv(data_frame_3, "table3_DNNW.CSV")
```

```{r}
DNnTotal_df <- rbind(data_frame_1,data_frame_2,data_frame_3)
DNnTotal_df
```
